...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 trigger `prevent_duplicate_seat` before insert
on `seat_position`
for each row
begin
	DECLARE num_rows INTEGER;
	SELECT COUNT(*) INTO num_rows FROM `trip` `trp` INNER JOIN `trip_in_reservation` `tir_new` ON `tir_new`.`trip_id` = `trp`.`id` AND NEW.`reservation_id` = `tir_new`.`reservation_id` INNER JOIN `trip_in_reservation` `tir` ON  `tir`.`trip_id` = `trp`.`id` AND `tir`.`reservation_id` <> NEW.`reservation_id` INNER JOIN `seat_position` `stp` ON `stp`.`reservation_id` = `tir`.`reservation_id` AND `stp`.`seat_name` = NEW.`seat_name`;
	IF num_rows > 0 THEN
...
-- Dump completed on 2013-02-27 15:48:59
