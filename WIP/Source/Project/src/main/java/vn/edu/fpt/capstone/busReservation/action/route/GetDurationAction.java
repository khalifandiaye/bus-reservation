...
public class GetDurationAction extends BaseAction {
   ...
   public String execute() {
      ...
      if (segmentBeans.size() != 0) {
         Long ltravelTime = (long) 0;
         List<SegmentTravelTimeBean> segmentTravelTimeBeans = segmentTravelTimeDAO
               .getTravelTimebyDate(segmentBeans.get(0).getId(), Calendar
                     .getInstance().getTime());
         if (segmentTravelTimeBeans.size() != 0) {
            ltravelTime = segmentTravelTimeBeans.get(0).getTravelTime();
         }
         Long hours = ltravelTime / (1000 * 60 * 60);
         Long minutes = (ltravelTime % (1000 * 60 * 60)) / (1000 * 60);
